...
class FlxCamera extends FlxBasic
{
	...
	/**
	 * The Array Of Filters To Be Applied To The Screen, Suggested To Use `addFilter()` / `addShader()` and `removeFilter()` / `removeShader()` instead of `setFilters()`
	 */
	public var camFilters:Array<BitmapFilter> = []; // for null handling, unstatic it as well while at it, I hate fucking static variables
	...
	{
		var f:ShaderFilter = new ShaderFilter(shader);
		if (shader != null)
			addFilter(f);
		return f;
	}
	...
	public function removeShader(shader:FlxShader):Bool
	{
		for (f in camFilters)
		{
			...
			{
				if (cast(f, ShaderFilter).shader == shader)
				{
					camFilters.remove(f);
					return true;
				}
	...
	public function drawPixels(?frame:FlxFrame, ?pixels:BitmapData, matrix:FlxMatrix, ?transform:ColorTransform, ?blend:BlendMode, ?smoothing:Bool = false,
			?shader:FlxShader, ?shaders:Array<FlxShader>):Void
	{
		if (FlxG.renderBlit)
		{
		...
		else
		{
			...
			#else
			var drawItem = startQuadBatch(frame.parent, isColored, hasColorOffsets, blend, smoothing, shader, shaders);
			#end
	...
	public function copyPixels(?frame:FlxFrame, ?pixels:BitmapData, ?sourceRect:Rectangle, destPoint:Point, ?transform:ColorTransform, ?blend:BlendMode,
			?smoothing:Bool = false, ?shader:FlxShader, ?shaders:Array<FlxShader>):Void
	{
		if (FlxG.renderBlit)
		{
			if (pixels != null)
	...
	override public function update(elapsed:Float):Void
	{
		...
		flashSprite.filters = filtersEnabled ? camFilters : null;

		updateFlashSpritePosition();
		updateShake(elapsed);

		// an easy to prepare widescreen lmfao
	 ...
	 */
	@:deprecated('Deprecated In Favor Of `addFilter` and `removeFilter`')
	...
	/**
	 * Adds A Filter To The Camera
	 * @param filter The BitmapFilter
	 * @author InvertZ
	 */
	public function addFilter(filter:BitmapFilter)
		if (filter != null)
			camFilters.push(filter);

	/**
	 * Removes A Filter From The Camera
	 * @param curFilter The BitmapFilter
	 * @author InvertZ
	 */
	public function removeFilter(curFilter:BitmapFilter)
		if (curFilter != null)
			for (filter in camFilters)
				if (Std.isOfType(filter, curFilter))
					camFilters.remove(curFilter);
	...
	public inline function containsPoint(oPoint:FlxPoint, width:Float = 0, height:Float = 0):Bool
	{
		...
		var contained = (point.x + width > viewMarginLeft)
			&& (point.x < viewMarginRight)
			&& (point.y + height > viewMarginTop)
			&& (point.y + fxShakePoint.y < viewMarginBottom);
		oPoint.putWeak();
		point.putWeak();
		return contained;
	}
